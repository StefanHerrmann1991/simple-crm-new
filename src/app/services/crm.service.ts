...
export class CrmService {
  ...
  subCustomersList() {
    const q = query(collection(this.firestore, 'customers'), orderBy('lastName'), limit(100));
  ...
  subContactsList() {
    const q = query(collection(this.firestore, 'contacts'), orderBy('contactPerson'), limit(100));
  ...
  async updateEntity(entity: any) {
    if (entity.id) {
      let docRef = doc(collection(this.firestore, this.getColIdFromEntity(entity)), entity.id)
  ...
  async deleteEntity(colId: 'customers' | 'contacts', docId: string) {
    await deleteDoc(doc(collection(this.firestore, colId), docId))
      .catch((err) => { console.log(err) })
  }
}
